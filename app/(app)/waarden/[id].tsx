...
import { formatWaardeDate, last14Days } from '@/lib/waarden';
...
export default function WaardeDetailScreen() {
  ...
  const acties = data.acties.filter((item) => item.waarde_id === waardeId);
  const barriers = data.barriers.filter((item) => item.waarde_id === waardeId);
  const checkins = data.checkins.filter((item) => item.waarde_id === waardeId);
      ...
      <View className="mx-auto w-full max-w-md gap-4">
        <View className="flex-row items-center gap-1">
          ...
          <Pressable
            ...
            accessibilityLabel={waarden.detail.editAction}
            onPress={() => router.push(`/waarden/edit/${waardeId}`)}
            className="shrink-0 rounded-lg border border-primary px-2.5 py-1.5 active:bg-primary-soft"
          >
        ...
        {waarde.beschrijving ? (
          <Text className="text-sm leading-5 text-text-subtle">{waarde.beschrijving}</Text>
        ) : null}
...
function PlanTab({
  ...
  onAdd: (waardeId: string, termijn: WaardeTermijn, actie: string) => void;
    ...
    <View className="gap-3.5">
      {blocks.map((block) => (
        <TermijnBlock
          ...
          onAdd={(text) => onAdd(waardeId, block.termijn, text)}
          onDelete={onDelete}
        />
      ))}
    </View>
...
function BarriersTab({
  ...
  function submit() {
    ...
    onAdd(waardeId, type, input, type === 'eigen' ? eigenLabel : undefined);
    setInput('');
